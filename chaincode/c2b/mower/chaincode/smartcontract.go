...
func (s *SmartContract) CreateSLA(ctx contractapi.TransactionContextInterface, id string, targetgrasslength float32, maxgrasslength float32, mingrasslength float32) error {
	exists, err := s.SLAExists(ctx, id)
	if err != nil {
		return err
	}
	if exists {
		return fmt.Errorf("the SLA %s already exists", id)
	}
	...
	slaValue, err := s.EvaluateSLA(ctx, &newSLA)
	...
	return ctx.GetStub().PutState(id, slaJSON)
}

func (s *SmartContract) EvaluateSLA(ctx contractapi.TransactionContextInterface, sla *SLA) (int, error) {

	spread := sla.MaxGrassLength - sla.MinGrassLength
	...
	targetFactor := 1.0 / sla.TargetGrassLength
	...
	monthlyCost := baseCost * costFactor
...
func (s *SmartContract) UpdateTargetGrassLength(ctx contractapi.TransactionContextInterface, id string, targetgrasslength float32) error {
	exists, err := s.SLAExists(ctx, id)
	if err != nil {
		return err
	}
	...
	newValue, err := s.EvaluateSLA(ctx, sla)
	if err != nil {
		return err
	}
...
func (s *SmartContract) UpdateGrassLengthInterval(ctx contractapi.TransactionContextInterface, id string, maxgrasslength float32, mingrasslength float32) error {
	exists, err := s.SLAExists(ctx, id)
	if err != nil {
		return err
	}
